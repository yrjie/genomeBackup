...
if len(sys.argv)<4:
    print 'Usage: allPeakTag outfile alpha [debug]'
...
class Peak:
    ...
    expR=0
    strand='.'
    def __init__(self, id, tagNum, lbRatio, rbRatio, lNum, rNum, expR):
        self.id=id
        self.tagNum=tagNum
...
#             allPeak.append(Peak(nowId, tagNum, 1.0*lbNum/tagNum, 1.0*rbNum/tagNum, lNum, rNum, expR))
            allPeak.append(Peak(nowId, atacSig, 1.0*lbNum/tagNum, 1.0*rbNum/tagNum, lNum, rNum, expR))
            nowId=id
            expR=float(temp[expC])
            atacSig=float(temp[atacC])
            tagNum=0
...
def printOut(outfile, pk):
    fo=open(outfile,'w')
    for x in pk:
        fo.write('\t'.join([x.id, str(x.sig), str(x.expR), x.strand])+'\n')
    fo.close()
