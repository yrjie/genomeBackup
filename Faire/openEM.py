...
def genDistrPN(peakSet, th, psCnt):
    num=len(peakSet)
    allBratio=[x.bRatio for x in peakSet]
    allTagnum=[x.tagNum for x in peakSet]
#     retPr=[[1]*binN for i in range(binN)]
    ...
    for i in range(binN):
        for j in range(binN):
            retPr[i][j]/=1.0*(num+numPs)
    
    return retPr

def genDistr(peakSet, th, psCnt, z):
    num=len(peakSet)
    allBratio=[x.bRatio for x in peakSet]
    allTagnum=[x.tagNum for x in peakSet]
#     retPr=[[1]*binN for i in range(binN)]
    retPr=copy.deepcopy(psCnt)
    if z==1:
        postPr=[x.z for x in peakSet]
    else:
        postPr=[1-x.z for x in peakSet]
    for i in range(num):
        for idx0, y in enumerate(th[0]):
            if allBratio[i]<=y:
                break
        for idx1, y in enumerate(th[1]):
            if allTagnum[i]<=y:
                break
        retPr[idx0][idx1]+=1*postPr[i]
    
    for i in range(binN):
        for j in range(binN):
            retPr[i][j]/=1.0*sum(postPr)
...
#     print psCnt
    edistrP=genDistrPN(posPk, th, psCnt)
    edistrN=genDistrPN(negPk, th, psCnt)
    
#     edistrP=genDistr(allPeak, th, psCnt, 1)
#     edistrN=genDistr(allPeak, th, psCnt, 0)
...
#         fo.write('\t'.join([x.id, str(x.z)])+'\n')
        fo.write('\t'.join([x.id, str(x.z), str(x.bRatio)])+'\n')
    fo.close()
