...
if __name__ == "__main__":
    print ''
    import sys
    wigs=Wigs('./',suppress=True)
    st=wigs.samplingTotal(exclude_low_percent=0,exclude_high_percent=10,bnum=100000)
    av=sum(st.values())/len(st.values())
    for name in st:print name,st[name],st[name]/av
