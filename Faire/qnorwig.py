...
def rawsort(ifile,sort_ofile,gfile,format,step=10,suppress=False,buffer=None):
    ...
    else:cmd='sort -r -n -s -k1 -k2 -o '+sort_ofile+' '+raw_ofile
    os.system(cmd)
    if format=='wig':
...
def refquantile(paths,ofile,gfile):
    ...
    for line in fi[files[0]]:
        ...
        fo.write(str(v)+'\t'+col[1]+'\t'+col[2]+'\t'+col[3]+'\n')
    fo.close()
    print 'time cost:',time()-tm
...
def qnorwig(paths,gfile,out_dir='wiq_result',ref=None,iformat='wig',rformat='wig',isorted=False,rsorted=False,step=10,suppress=False,buffer=30000000):
    #buffer unit is 1024
    alltime=time()
    ...
    if ref==None:
        refpath=os.path.join(out_dir,'reference.sort.wiq')
...
def run_qnorwig(command='wiq'):
    ...
    if (len(sys.argv)<2) and ('-h' not in sys.argv) and ('--help' not in sys.argv):
        ...
        print "\nusage:\npython danpos.py wiq  <file_path> [optional arguments]\n\nfor more help, please try: python danpos wiq -h\n"
        return 0
    ...
    parser.add_argument('file_paths',default=None,\
                        ...
                        Each path could point to a file or a directory that contail the files. use ':'\
                        to seperate paths, e.g. 'Path_A:path_B")
    parser.add_argument('genome_description_file',default=None,\
                        help="A text file with each line describe length of a chrosome, e.g. 'chr1 10043'")
    ...
    parser.add_argument('--input_format', dest="iformat",metavar='',default='wig',\
                        help="The data format in input files, can be 'wig' or 'wiq'")
    parser.add_argument('--reference_format', dest="rformat",metavar='',default='wig',\
                        help="The data format in reference file, can be 'wig' or 'wiq'")
    parser.add_argument('--input_sorted', dest="isorted",metavar='',default=0,\
                        help="set to 1 if the input file in wiq format and sorted.")
    parser.add_argument('--reference_sorted', dest="rsorted",metavar='',default=0,\
                        help="set to 1 if the reference file is in wiq format and sorted.")
    parser.add_argument('--step', dest="step",metavar='',default=10,\
                        help="the step size in wiggle format data.")
    parser.add_argument('--step', dest="step",metavar='',default=10,\
                        help="the step size in wiggle format data.")
    parser.add_argument('--buffer_size', dest="buffer",metavar='',default=10,\
...
if __name__ == "__main__":
    #ifile,gfile=sys.argv[1],'/lilab/kaifuc/ref_data/genomes/hg19/hg19.chrom.sizes.removeUnknown'
    ifile,gfile=sys.argv[1],'/lilab/kaifuc/ref_data/genomes/sacCer3/sacCer3.chrSizes.xls' #'hg19.chrom.sizes.removeUnknown'
    qnorwig(ifile,gfile,out_dir='qnor')
    
    #qnorwig(ifile,gfile,out_dir='qnor',ref='reference.sort.wiq',iformat='wiq',rformat='wiq',isorted=True,rsorted=True)
